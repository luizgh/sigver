...
from sigver.adversarial.pgd_max_l2 import PGD_max_l2_truncate
...
def train(base_model: torch.nn.Module,
    ...
    for epoch in range(args.epochs):
        ...
        train_epoch(train_loader, base_model, classification_layer, forg_layer, attack,
...
def train_epoch(train_loader: torch.utils.data.DataLoader,
    ...
    #TODO refactor - we should not create this in every epoch
    classification_model = nn.Sequential(base_model, classification_layer)
    ...
    for batch in train_loader:
        ...
        # Forward propagation
        adv = attack(classification_model, x, y)
        features = base_model(adv)
        ...
        if callback and step % 10 == 0:
            with torch.no_grad():
                logits_clean = classification_layer(base_model(x[yforg == 0]))
            ...
            callback.scalars(['closs_clean', 'closs_adv'], iteration, [loss_clean.detach(), class_loss.detach()])

            pred = logits.argmax(1)
            acc = y[yforg == 0].eq(pred).float().mean()
            callback.scalars(['acc_clean', 'acc_addv'], epoch + (step / n_steps), [acc_clean, acc.detach()])
            if args.forg:
...
def main(args):
    ...
    if args.test:
        ...
        attack = PGD_max_l2_truncate(steps=args.attack_steps, max_l2=args.attack_l2,
                                     device=device,
                                     num_classes=n_classes, fast=True,
                                     image_constraints=(0, 1))

        train(base_model, classification_layer, forg_layer, attack.attack, train_loader, val_loader,
...
if __name__ == '__main__':
    ...
    argparser.add_argument('--attack-steps', default=200, type=int)
    argparser.add_argument('--attack-l2', default=20, type=int)
